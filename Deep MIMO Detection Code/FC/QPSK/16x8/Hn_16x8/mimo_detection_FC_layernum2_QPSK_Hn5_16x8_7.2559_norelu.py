...
LOGDIR="path/layernum2/16x8/Hn5-7.2559-norelu"
K=8  #Number of users
...
Hdata=scipy.io.loadmat('Hn5_16x8_7.2559')
H=Hdata['Hn5']
...
with tf.name_scope('loss'):
	lossfunction=tf.log(1.00)*tf.reduce_sum(tf.squared_difference(tf.transpose(x),x1))/tf.reduce_sum(tf.squared_difference(tf.transpose(x),xwave))\
             	...
             	# +tf.log(3.00)*tf.reduce_sum(tf.squared_difference(tf.transpose(x_real),x3))/tf.reduce_sum(tf.squared_difference(tf.transpose(x_real),xwave))\
             	# +tf.log(4.00)*tf.reduce_sum(tf.squared_difference(tf.transpose(x_real),x4))/tf.reduce_sum(tf.squared_difference(tf.transpose(x_real),xwave))
...
for i1 in range (8,16):
	...
	ber[:,indexcount]=[ber_layer1, ber_layer2]
...
scipy.io.savemat('result/layernum2/16x8/norelu/Hn5/ber.mat',{'ber':ber})
scipy.io.savemat('result/layernum2/16x8/norelu/Hn5/x2.mat', {'x2': xkout2bit_seq})
scipy.io.savemat('result/layernum2/16x8/norelu/Hn5/source.mat', {'source': source_test})
scipy.io.savemat('result/layernum2/16x8/norelu/Hn5/berdecorSet.mat', {'berdecorSet': berdecorSet})

sess.close()
print("layernum2_Hn5_norelu_5000_16*8_end")
print("end")
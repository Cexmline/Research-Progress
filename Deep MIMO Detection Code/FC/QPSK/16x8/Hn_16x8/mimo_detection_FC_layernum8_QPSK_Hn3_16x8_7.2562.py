...
LOGDIR = "path/layernum8/16x8/Hn3-7.2562"
K = 8  # Number of users
...
Hdata = scipy.io.loadmat('Hn3_16x8_7.2562')
H = Hdata['Hn3']
...
with tf.name_scope('layer5parameter'):
    # w33=tf.Variable(tf.zeros([2*K,5*K]))
    w14 = tf.Variable(tf.truncated_normal([6 * K, 6 * K], stddev=1, seed=1))
    b14 = tf.Variable(tf.constant(0.1, shape=[6 * K, 1]))
...
with tf.name_scope('layer6parameter'):
    # w33=tf.Variable(tf.zeros([2*K,5*K]))
    w15 = tf.Variable(tf.truncated_normal([6 * K, 6 * K], stddev=1, seed=1))
    b15 = tf.Variable(tf.constant(0.1, shape=[6 * K, 1]))
...
with tf.name_scope('layer6parameter'):
    # w33=tf.Variable(tf.zeros([2*K,5*K]))
    w16 = tf.Variable(tf.truncated_normal([6 * K, 6 * K], stddev=1, seed=1))
    b16 = tf.Variable(tf.constant(0.1, shape=[6 * K, 1]))
    w26 = tf.Variable(tf.truncated_normal([2 * K, 6 * K], stddev=1, seed=1))
    b26 = tf.Variable(tf.constant(0.1, shape=[2 * K, 1]))
    # w33=tf.Variable(tf.truncated_normal([2*K,5*K],stddev=1,seed=1))
    # b33=tf.Variable(tf.constant(0.1,shape=[2*K,1]))
...
with tf.name_scope('layer6parameter'):
    # w33=tf.Variable(tf.zeros([2*K,5*K]))
    w17 = tf.Variable(tf.truncated_normal([6 * K, 6 * K], stddev=1, seed=1))
    b17 = tf.Variable(tf.constant(0.1, shape=[6 * K, 1]))
    w27 = tf.Variable(tf.truncated_normal([2 * K, 6 * K], stddev=1, seed=1))
    b27 = tf.Variable(tf.constant(0.1, shape=[2 * K, 1]))
    # w33=tf.Variable(tf.truncated_normal([2*K,5*K],stddev=1,seed=1))
    # b33=tf.Variable(tf.constant(0.1,shape=[2*K,1]))
    tf.summary.histogram("weights1", w17)
    tf.summary.histogram("weigths2", w27)
    # tf.summary.histogram("weights3", w33)
    tf.summary.histogram("biases1", b17)
    tf.summary.histogram("biases2", b27)
...
for i in range(STEPS):
    ...
    if i % 5000 == 0:
        ...
        print("x2:", sess.run(x2, feed_dict={y: y_}))
...
xkout5 = np.zeros((2 * K, testsymbolnum))
xkout6 = np.zeros((2 * K, testsymbolnum))
xkout7 = np.zeros((2 * K, testsymbolnum))
...
for i1 in range(8, 16):
    source_test, xcomplex_test, ycomplex_test, x_test, y_test = generate_testdata(testsymbolnum, K, N, H, modType,
                                                                                  modName, i1)
    ...
    xkout7_complex = xkout7[0:K, :] + 1j * xkout7[K:2 * K, :]
    xkout7_complex_seq = np.reshape(np.transpose(xkout7_complex), (testsymbolnum * K, 1))
    ...
    xkout5bit_seq = demodulation(xkout5_complex_seq, testsymbolnum * K, modType)
    xkout6bit_seq = demodulation(xkout6_complex_seq, testsymbolnum * K, modType)
    xkout7bit_seq = demodulation(xkout7_complex_seq, testsymbolnum * K, modType)
    ...
    ber_layer5 = np.sum(abs(xkout5bit_seq - source_test)) / sourcelength
    ber_layer6 = np.sum(abs(xkout6bit_seq - source_test)) / sourcelength
    ber_layer7 = np.sum(abs(xkout7bit_seq - source_test)) / sourcelength
    ber_layer8 = np.sum(abs(xkout8bit_seq - source_test)) / sourcelength
    ber[:, indexcount] = [ber_layer1, ber_layer2, ber_layer3, ber_layer4,ber_layer5, ber_layer6,ber_layer7, ber_layer8]
...
scipy.io.savemat('result/layernum8/16x8/Hn3/ber.mat', {'ber': ber})
scipy.io.savemat('result/layernum8/16x8/Hn3/xkout8_complex_seq.mat', {'xkout8_complex_seq': xkout8_complex_seq})
scipy.io.savemat('result/layernum8/16x8/Hn3/x8.mat', {'x8': xkout6bit_seq})
scipy.io.savemat('result/layernum8/16x8/Hn3/source.mat', {'source': source_test})
scipy.io.savemat('result/layernum8/16x8/Hn3/berdecorSet.mat', {'berdecorSet': berdecorSet})

sess.close()
print("layernum8_Hn3_5000_16*8_end")
print("end")

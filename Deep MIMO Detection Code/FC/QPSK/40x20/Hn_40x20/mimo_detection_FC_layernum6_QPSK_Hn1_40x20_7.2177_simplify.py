...
with tf.name_scope('layer1parameter'):
    ...
    b10 = tf.Variable(tf.constant(0.1, shape=[2 * K, 1]))
...
with tf.name_scope('layer2parameter'):
    ...
    b11 = tf.Variable(tf.constant(0.1, shape=[2 * K, 1]))
...
with tf.name_scope('layer3parameter'):
    ...
    b12 = tf.Variable(tf.constant(0.1, shape=[2 * K, 1]))
...
with tf.name_scope('layer4parameter'):
    ...
    b13 = tf.Variable(tf.constant(0.1, shape=[2 * K, 1]))
...
with tf.name_scope('layer5parameter'):
    ...
    b14 = tf.Variable(tf.constant(0.1, shape=[2 * K, 1]))
...
with tf.name_scope('layer6parameter'):
    ...
    b15 = tf.Variable(tf.constant(0.1, shape=[2 * K, 1]))
...
for i in range(STEPS):
    ...
    if i % 5000 == 0:
        ...
        print("x6:", sess.run(x6, feed_dict={y: y_}))
...
for i1 in range(8, 16):
    ...
    ber[:, indexcount] = [ber_layer1, ber_layer2, ber_layer3, ber_layer4,ber_layer5, ber_layer6]
...
scipy.io.savemat('result/layernum6/40x20/simplify/Hn1/ber.mat', {'ber': ber})
scipy.io.savemat('result/layernum6/40x20/simplify/Hn1/xkout6_complex_seq.mat', {'xkout6_complex_seq': xkout6_complex_seq})
scipy.io.savemat('result/layernum6/40x20/simplify/Hn1/x6.mat', {'x6': xkout6bit_seq})
scipy.io.savemat('result/layernum6/40x20/simplify/Hn1/source.mat', {'source': source_test})
scipy.io.savemat('result/layernum6/40x20/simplify/Hn1/berdecorSet.mat', {'berdecorSet': berdecorSet})

sess.close()
print("layernum6_Hn1_simplify_5000_40*20_end")
print("end")

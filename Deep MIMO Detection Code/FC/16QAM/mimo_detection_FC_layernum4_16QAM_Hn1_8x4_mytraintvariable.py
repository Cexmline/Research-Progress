...
LOGDIR="path/layernum4/8x4/Hn1-traintv"
K=4  #Number of users
...
Hdata=scipy.io.loadmat('Hn1_8x4_3.0699')
H=Hdata['Hn1']
...
with tf.name_scope('layer1'):
    ...
    t0=tf.Variable(0.1,name="t0")
    t01=tf.Variable(0.2,name="t01")
    x1 = 0.316228*((tf.nn.relu(cal2 + t0) - tf.nn.relu(cal2 - t0)) / (2*abs(t0))+(tf.nn.relu(cal2 + t01-0.632456) - tf.nn.relu(cal2 - t01-0.632456)) / abs(t01))\
    -0.316228*((tf.nn.relu(-cal2 + t0) - tf.nn.relu(-cal2 - t0)) / (2*abs(t0))+(tf.nn.relu(-cal2 + t01-0.632456) - tf.nn.relu(-cal2 - t01-0.632456)) / abs(t01))
...
with tf.name_scope('layer2'):
    ...
    t1=tf.Variable(0.1,name="t1")
    t11=tf.Variable(0.2,name="t11")
    x2 = 0.316228*((tf.nn.relu(cal2_1 + t1) - tf.nn.relu(cal2_1 - t1)) / (2*abs(t1))+(tf.nn.relu(cal2_1 + t11-0.632456) - tf.nn.relu(cal2_1 - t11-0.632456)) / abs(t11))\
    -0.316228*((tf.nn.relu(-cal2_1 + t1) - tf.nn.relu(-cal2_1 - t1)) / (2*abs(t1))+(tf.nn.relu(-cal2_1 + t11-0.632456) - tf.nn.relu(-cal2_1 - t11-0.632456)) / abs(t11))
...
with tf.name_scope('layer3'):
    ...
    t2=tf.Variable(0.1,name="t2")
    t21=tf.Variable(0.2,name="t21")
    x3 = 0.316228*((tf.nn.relu(cal2_2 + t2) - tf.nn.relu(cal2_2 - t2)) / (2*abs(t2))+(tf.nn.relu(cal2_2 + t21-0.632456) - tf.nn.relu(cal2_2 - t21-0.632456)) / abs(t21))\
    -0.316228*((tf.nn.relu(-cal2_2 + t2) - tf.nn.relu(-cal2_2 - t2)) / (2*abs(t2))+(tf.nn.relu(-cal2_2 + t21-0.632456) - tf.nn.relu(-cal2_2 - t21-0.632456)) / abs(t21))
#
#layer4
with tf.name_scope('layer4'):
    ...
    t3 = tf.Variable(0.1,name="t3")
    t31 = tf.Variable(0.2,name="t31")
    x4 = 0.316228*((tf.nn.relu(cal2_3 + t3) - tf.nn.relu(cal2_3 - t3)) / (2*abs(t3))+(tf.nn.relu(cal2_3 + t31-0.632456) - tf.nn.relu(cal2_3 - t31-0.632456)) / abs(t31))\
    -0.316228*((tf.nn.relu(-cal2_3 + t3) - tf.nn.relu(-cal2_3 - t3)) / (2*abs(t3))+(tf.nn.relu(-cal2_3 + t31-0.632456) - tf.nn.relu(-cal2_3 - t31-0.632456)) / abs(t31))
...
for i in range(STEPS):
    ...
    if i%5000==0:
        ...
        print("t01:", sess.run(t01))
        print("t1:", sess.run(t1))
        print("t11:", sess.run(t11))
        print("t2:", sess.run(t2))
        print("t21:", sess.run(t21))
        print("t3:", sess.run(t3))
        print("t31:", sess.run(t31))
...
scipy.io.savemat('result/layernum4/8x4/Hn1-traintv/ber.mat',{'ber':ber})
scipy.io.savemat('result/layernum4/8x4/Hn1-traintv/x4.mat', {'x4': xkout4bit_seq})
scipy.io.savemat('result/layernum4/8x4/Hn1-traintv/source.mat', {'source': source_test})
scipy.io.savemat('result/layernum4/8x4/Hn1-traintv/berdecorSet.mat', {'berdecorSet': berdecorSet})

sess.close()
print("layernum4_Hn1-traintv_5000_8x4_end")
print("end")